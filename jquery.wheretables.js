 ...
 *
 * [sc]: https://github.com/where/jquery-plugins/blob/master/jquery.wheretables.js
 */
...
// ## Class Definition
  ...
  // ### The jQuery.whereTables Method

  /**
   * This is where we introduce the `jQuery.whereTables` method. In addition to
   * the jQuery method, I also assign it a shorthand reference of `WT`. This
   * method takes one parameter (`request`) which is explained below.
   */

  var WT = $.fn.whereTables = function( request ){

    // If `request` is a non-blank string
    if ( "string" === typeof request && "" !== request ) {
      // and if the `request` is a string of `api`, return this instance's
      // `api` object.
      if ( request === 'api' ) return $( this[0] ).data('whereTables');
      var args = Array.prototype.slice.call(arguments, 1);
      return this.each( function(){
        var api = $( this ).data('whereTables');
        // If `request` matches an `api` method, call that method.
        if ( api && api[request] ){
          api[request].apply( api[request], args );
        } else {
          if ( 'undefined' === typeof api )
            // Otherwise either the target element has no `api` object
            WT.log ( 'whereTables not initialized on %o', this );
          else
            // or there is no `api` method that matches `request`.
            WT.log ( 'Cannot find API method: %s', request );
        }
      } );
    }

    // `request` isn't defined, so we should just initialize instead.

    // Return everything in the original request to allow method-chaining.
    return this.each( function(){
      ...
      // Create a new instance of the `whereTables` "class."
      var obj = new whereTables( this, id );
      // Assign a reference to this class instance to the target element,
      // itself.
  ...
  // Helper logging function. **Note**: this isn't a good permanent solution.
  WT.log = function( ){
...
/**
 * Call the closure at the beginning of this file with a reference to `jQuery`
 * and the `window` (pulling `window` into the local scope is a minor
